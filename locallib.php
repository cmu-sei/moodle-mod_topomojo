...
/**
 * Gets the launchpoint URL stored for a gamespace.
 *
 * @param string $gamespaceid The TopoMojo gamespace ID.
 * @return string|null The stored launchpoint URL, or null when none is available.
 */
function get_topomojo_gamespace_launchpoint_url($gamespaceid) {
    global $DB;

    $sql = "SELECT id, launchpointurl
              FROM {topomojo_attempts}
             WHERE " . $DB->sql_compare_text('eventid') . " = " . $DB->sql_compare_text(':eventid') . "
          ORDER BY timemodified DESC, id DESC";

    $attempts = $DB->get_records_sql($sql, ['eventid' => $gamespaceid], 0, 1);
    $attempt = reset($attempts);
    if (!$attempt || empty($attempt->launchpointurl)) {
        return null;
    }

    return $attempt->launchpointurl;
}

/**
 * Builds a TopoMojo invitation URL from a launchpoint URL.
 *
 * @param string|null $launchpointurl The launchpoint URL generated by TopoMojo.
 * @param string $code The invitation code.
 * @return string The invitation URL.
 */
function build_topomojo_invite_url($launchpointurl, $code) {
    if (empty($launchpointurl)) {
        $launchpointurl = get_topomojo_base_url() . '/lp/';
    }

    $url = new moodle_url($launchpointurl);
